            ...
            with open(
                ...
                for line in f.readlines():
                    ...
                    if len(tmp) == 1:
                        ...
                        points = tmp[:4]
                        s = []
                        s.append([int(points[0]),int(points[1])])
                        s.append([int(points[2]),int(points[1])])
                        s.append([int(points[2]),int(points[3])])
                        s.append([int(points[0]),int(points[3])])
                        result = {"transcription": tmp[4][1:-1], "points": s}
                        label.append(result)
                    if len(tmp) == 9:   
                        points = tmp[:8]
                        s = []
                    ...
                    #     s.append([int(points[2]),int(points[3])])
                    #     s.append([int(points[0]),int(points[3])])
                    #     result = {"transcription": tmp[4][2:-1], "points": s}
                    #     label.append(result)
