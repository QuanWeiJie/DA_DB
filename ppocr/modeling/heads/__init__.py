...
def build_head(config):
    ...
    from .det_db_head_domain import DBHead_domain
    from .det_east_head_domain import EASTHead_domain
    from .det_my_east_head import EASTHead
    ...
    support_dict = [
        ...
        'DRRGHead', 'CANHead','DBHead_domain'
    ]
    ...
    if 'DB Instance alignment' in config:
        if config['DB Instance alignment']:
            module_class = eval('DBHead_domain')(**config)
            need = False
    elif 'EAST Instance alignment' in config:
        if config['EAST Instance alignment']:
            module_class = eval('EASTHead_domain')(**config)
            need = False
    if need:
