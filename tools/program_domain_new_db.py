...
# 在baseline的基硢�上加 全局级对齐模块��实例级对齐模块、一致��正则化〄1�7
# 全局级对齄1�7 使用bce计算loss
# 实例级对齄1�7  focal loss  计算log时没有对结果做限制，会出现负无穷
    #   源域使用threshold mask做实例级掩码〄1�7  
    #   目标域使用预测的binarize map做掩码（要不要过阈��，其实可以0.3就行＄1�7
# 全局级对齄1�7 预测 扩展刄1�7 map大小。����像約1�7
...
class ArgsParser(ArgumentParser):
    def __init__(self):
        ...
        self.add_argument("-c", "--config",default='/tmp/Experiments/PaddleOCR/domain/domain_config/aT-SNE.yaml',help="configuration file to use")
        self.add_argument(
...
def train(config,
    ...
    #     if len(data_x)==100:
    #         break
    # for idx, batch in enumerate(target_dataloader): 
    #     preds = model(batch[0])
    #     p = preds.tolist()
    #     data_x.append(p)
    #     if len(data_x)==200:
    ...
    # embeding = TSNE().fit(np.array(data_x))
    # file_name = '20x20mean13-mp_new2.txt'
    # with open(file_name,'w') as file:
    #     # file.readlines(data_x)
    #     for i in embeding:
    ...
    # ax.scatter(embeding[:100,0],embeding[:100,1],c='r')

    # ax.scatter(embeding[100:,0],embeding[100:,1],c='b')
    # ax.get_xaxis().set_visible(False)
    # ax.get_yaxis().set_visible(False)
    # plt.legend(['ICDAR2013','铭牌'])
    # plt.title("数据分布对比")
    # plt.savefig("icdar13-mp_db++_new2.png")
    # # T-SNE end
    ...
    for step in range(start_step, total_step + 1):
        ...
        if (target_dataloader is None and 'Target' not in config) or step < burn_up_step:  
            # not has Target dataset 或��在磨合阶段 ,use source dataset for training
            ...
            total_data = paddle.concat(([train_data[0],target_data[1]]),axis=0)
            total_preds = model(total_data)

            # 源域损失
            # total_preds {'maps': y,'global_domain_cls':global_domain_cls,'domain_cls':domain_cls} 
            source_preds = dict()
            source_preds["maps"] = total_preds["maps"][:train_data[0].shape[0],:,:,:]
            ...
            if teacher_model is not None and config['Architecture']['Pseudo label']:
                ...
                # 目标域弱监督损失
                # pseudo_label = paddle.to_tensor(preds['maps'][:,0,:,:] > 0.3,dtype=paddle.float16) * preds['maps'][:,0,:,:]
                pseudo_label = preds['maps'][:,0,:,:]  # binary map 二��图
                    # total_preds["maps"] = {shrink_maps, threshold_maps, binary_maps}
                # target_preds = total_preds["maps"][train_data[0].shape[0]:,0,:,:]  # shrink map 概率囄1�7
                target_preds = total_preds["maps"][train_data[0].shape[0]:,2,:,:]  # binarize map 二��图
                target_loss = DiceLoss(target_preds,pseudo_label)
                total_loss +=  target_loss * (step / float(total_step))
                print_loss['target_loss'] = target_loss
            if config['Architecture']['DB Imgage alignment']:
                # 全局对齐损失
                ...
                total_loss += (source_global_loss + target_global_loss)* 0.001
                print_loss['s_g'] = source_global_loss
                print_loss['t_g'] = target_global_loss

            if config['Architecture']['DB Instance alignment']:
                # 实例对齐损失
                # train_data = { strong_image, threshold_map, threshold_mask, shrink_map,shrink_mask}
                source_mask = train_data[3]  # shrink map 作为mask   shape 8*1*640*640
                ...
                target_mask = paddle.to_tensor(preds['maps'][:,0,:,:] > 0.3,dtype=paddle.float32) # pred_mask
                ...
                total_loss += (source_instance_loss + target_instance_loss)*0.001
